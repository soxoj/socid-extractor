...
| [`cli.py`](../socid_extractor/cli.py) | Argument parsing, logging setup, cookie loading, optional activation, orchestration of `parse` / `mutate_url` / `extract` for `--url` or `--file`. |
